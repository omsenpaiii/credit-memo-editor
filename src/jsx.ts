...
type Equal<A, B, T = true, F = false> = (<P>() => P extends A ? 1 : 2) extends <
  P
>() => P extends B ? 1 : 2
  ? T
  : F;
...
type AssignableParts<T> = T extends {} ? Omit<T, ReadonlyKeys<T>> : never;
...
type MapIntrinsicElements<T> = {
  [K in keyof T]: T[K] extends {}
    ? Partial<AssignableParts<T[K]> & Extras>
    : never;
};
...
function Fragment(): JSX.Element {
  throw new Error('`Fragment` should not be called');
}
