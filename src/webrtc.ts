...
function setupChannel(
  ...
  channel.onmessage = (e) => {
    ...
    else if (type === 'update') Y.applyUpdate(doc, databytes, 'remote');
    else if (type === 'sync') {
      const update = Y.encodeStateAsUpdate(doc, databytes);
      send('update', update);
      const aware = yawareness.encodeAwarenessUpdate(awareness, [
  ...
  channel.onerror = channel.onclose = () => {
    ...
    yawareness.removeAwarenessStates(
      awareness,
      [...awareness.getStates()]
        .filter(([_, v]) => v.clientId === remoteId)
        .map((x) => x[0]),
      'remote'
    );
  };
...
export function setupWebrtc(
  ...
  reply.onmessage = (e) => {
    ...
    if (offer) {
      ...
      conn.onconnectionstatechange = () => {
        if (
          ['closed', 'disconnected', 'failed'].includes(conn.connectionState)
        ) {
          console.log(conn.connectionState, 'from remote:', remoteId);
          connections.delete(remoteId);
