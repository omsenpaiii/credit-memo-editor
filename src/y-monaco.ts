...
function createmonacoSelection(
    ...
    function renderDecorators() {
      if (awareness) {
        ...
        states.forEach((state, clientID) => {
          ...
          if (state.name) widget.setLabel(state.name);
          if (state.color) widget.setColor(state.color);
          if (state.selection) {
            const selection = createmonacoSelection(
              editor,
              ytext,
              state.selection
            );
      ...
      this.disposes.push(
        editor.onDidChangeCursorSelection(() => {
          awareness.setLocalStateField(
            'selection',
            createRelativeSelection(editor, ytext)
          );
        })
      );
